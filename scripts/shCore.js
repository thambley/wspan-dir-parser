...
function splitLines(block) {
	//add spaces before and after block to create empty elements if necessary
	var tempString = ' ' + block + ' ';
	var tempArray = tempString.split(/\r?\n/);
	//remove leading space from first element:
	tempElement = tempArray[0];
	tempElement = (tempElement == ' ') ? '' : tempElement.substr(1,tempElement.length-1);
	tempArray[0] = tempElement;
	//remove trailing space from last element:
	var tempElement = tempArray[tempArray.length -1];
	tempElement = (tempElement == ' ') ? '' : tempElement.substr(0,tempElement.length-1);
...
function findBrush(alias, showAlert) {
	...
	if (result == null && showAlert) {
		custom_alert(sh.config.strings.noBrush + alias);
	}
...
sh.HtmlScript = function(scriptBrushName) {
	...
	if (scriptBrush.htmlScript == null) {
		custom_alert(sh.config.strings.brushNotHtmlScript + scriptBrushName);
		return;
	}
...
sh.Highlighter.prototype = {
	...
	getHtml: function(code) {
		...
		matches = this.findMatches(this.regexList, code); //HERE
